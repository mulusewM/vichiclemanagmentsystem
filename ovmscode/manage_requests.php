...
function isdelete()
  {
   ...
   if(!d)
   {
    alert(window.location='deleterequests.php');
   }
   else
   {
   return false;
...
<div id="tooplate_wrapper">
    ...
    <div id="tooplate_menu">
		...
		<?php		
			$av=mysql_query("select *from servicerequests where RequestStatus='NO'");
  			$countav=mysql_num_rows($av);			
			echo '<font color="red" >You have &nbsp;'.$countav. '&nbsp; New message.</font>' ;
			echo"<br><br>";
			?>
	<table style="color:black" border='1' width='900' height='200' align='center'>
<tr>
...
<?php
include("connection.php");  
$result = mysql_query("SELECT * FROM servicerequests ORDER BY SID DESC");
...
$status=$row['RequestStatus'];
						...
						{
						?>
                       			 <a href="statusServices.php?status=<?php echo $row['SID'];?>" onclick="return confirm('Can you  Give service  to us (<?php echo $SID?>)');">
						...
						{
						?>
                       			 <a href="statusServices.php?status=<?php echo $row['SID'];?>" onclick="return confirm('Can rejectss Give service  to us (<?php echo $SID?>)');"> 
						...
						<?php
						print("<td style='height:30px;'><a href = 'deleterequests.php?key=".$ctrl."'><img width='15px' height='15px' src = 'images/actions-delete.png' title='Delete' onclick='isdelete();'><a href = 'manage_requests.php?user_id=".$ctrl."'></a></td>");
	                    ?>						
					</tr>
<?php
  }
