...
<form action="officerupdate.php" method="post">
<center>
...
<table style="color:black">
		 ...
		 <td style="padding-top:12px;"><input type="password" name="confirm_password" pattern="\w{8,100}" required x-moz-errormessage="Please Enter Your password at least 8 characters " title="Please Enter Re_new password between at least 8  characters"></td>
		 </tr>
</table>	
</center>  
...
if(mysql_num_rows($result)==1){
    if($newpass!=$confirmpass){
		   ...
		   echo' <meta content="5;officerupdate.php" http-equiv="refresh" />';
		   }
		   else
		   {
  $query="update users set password='{$newpass}' where password='{$oldpass}'";
        $result=mysql_query($query);
	    echo "<script>alert(' You are  sucessfully  updated!');</script>".mysql_error(); 
         echo' <meta content="5;officerupdate.php" http-equiv="refresh" />';  
   }
   }
else
