...
//$ctrl=$_REQUEST['vid'];
$ctrl=$_SESSION['SID'];
$query="SELECT * FROM assignvehicless where SID='{$ctrl}'";
...
?>
  <form id="form1" method="POST" action="open_notificationstaff.php"  onsubmit='return formValidation()'>
...
?>
 ...
 <?php
  if(isset($_POST['update']))
  {
				$status=$_POST['status'];
				$SID=$_POST['SID'];
  $update = mysql_query("update assignvehicless set status='read'WHERE SID='{$SID}'") or die(mysql_error());
  if(!$update)
  {
  ...
  {
echo "<script>window.location='notifcationstaff.php';</script>";
  }}
  ?>
  </form> 	
				<br><br>
